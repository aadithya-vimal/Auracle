...
export default function App() {
    ...
    // Use CUMULATIVE stats for main display to ensure non-zero data
    const cumulative = engineData.cumulative ?? {}
    const botsDetected = cumulative.bots_detected ?? engineData.bots_detected ?? 0
    const spamFiltered = cumulative.spam_filtered ?? engineData.spam_filtered ?? 0
    const cleanTweets = cumulative.clean_tweets ?? engineData.clean_tweets ?? 0
    const outliers = cumulative.outliers_flagged ?? engineData.outliers_found ?? 0
    const duplicates = cumulative.duplicates_removed ?? engineData.duplicates_removed ?? 0
